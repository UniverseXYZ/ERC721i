...
contract ERC721I is ERC165, ERC721Consumable {
  ...
  ERC721ICore erc721ICore;
  DynamicRoyalties royalties;

  constructor(string memory name, string memory symbol) ERC721(name, symbol) {
    erc721ICore = new ERC721ICore(address(this));
  ...
  modifier onlyDAO() {
    require(msg.sender == erc721ICore.getDAOAddress(), "Wrong address");
    _;
  }

  function transferDAOownership(address payable _daoAddress) public onlyDAO {
    erc721ICore.setDAOAddress(_daoAddress);
  }
  ...
  function mint(
    ...
    erc721ICore.mint(_currentVersion, _assets, _metadataValues, _licenseURI, _editions);

    royalties._registerFees(erc721ICore.getTokenCounter(), _fees, _editions);

    for (uint256 i = 0; i < _editions; i++) {
      uint256 newTokenId = erc721ICore.getTokenCounter();
      _mint(msg.sender, newTokenId);
      emit TokenMinted(newTokenId, _assets[0][0], msg.sender, block.timestamp);
      if (i != (_editions - 1)) erc721ICore.tokenCounterIncrement();
    }
  }

  function getTokenCreator(uint256 tokenId) public view returns (address) {
    require(_exists(tokenId), "Nonexistent token");
    return erc721ICore.getTokenData(tokenId).tokenCreator;
  }
  ...
  function withdrawERC20(uint256 amount, address tokenAddress) public onlyDAO {
    ...
    token.transfer(erc721ICore.getDAOAddress(), amount);
  }
}